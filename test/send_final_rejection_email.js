...
async function sendFinalRejectionEmail() {
  console.log('📧 Sending final rejection email with production domain...')
  ...
  try {
    ...
    console.log('🔗 Using production URL:', updateApplicationUrl)
  ...
  <div style="background-color: #f5f5f5; padding: 40px 20px;">
    ...
    const response = await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        // Disable click tracking to avoid Resend wrapper URLs
        tags: [
          {
            name: 'category',
            value: 'designer-rejection'
          }
        ],
        headers: {
    ...
    if (response.ok) {
      console.log('✅ Final rejection email sent successfully!')
      console.log('📧 Check designbattlefield@gmail.com inbox')
      console.log('🔗 Production URL:', updateApplicationUrl)
      console.log('📝 Email ID:', result.id)
      console.log('\n💡 The link will now use onedesigner.app domain')
    } else {
