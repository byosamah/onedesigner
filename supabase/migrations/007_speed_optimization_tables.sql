-- Migration: Speed Optimization Tables
...
-- Designer Quick Stats Materialized View for instant access
CREATE MATERIALIZED VIEW IF NOT EXISTS designer_quick_stats AS
SELECT 
  d.id as designer_id,
  COUNT(DISTINCT m.id) as total_projects,
  COALESCE(AVG(d.rating), 4.5) as avg_rating,
  COALESCE(d.project_completion_rate / 100.0, 0.95) as completion_rate,
  COALESCE(d.avg_project_duration_days / 24.0, 24) as avg_response_time_hours,
...
GROUP BY d.id, d.rating, d.project_completion_rate, d.avg_project_duration_days;
...
CREATE INDEX IF NOT EXISTS idx_designer_budget ON designers(preferred_project_size);
CREATE INDEX IF NOT EXISTS idx_designer_timeline ON designers(preferred_timeline);
...
AFTER UPDATE OF styles, industries, specializations, design_philosophy, portfolio_keywords
ON designers
FOR EACH ROW
