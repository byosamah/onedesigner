...
export async function POST(request: NextRequest) {
  try {
    ...
    // Environment check - NO FALLBACK SECRETS for security
    const apiKey = process.env.LEMONSQUEEZY_API_KEY
    const storeId = process.env.LEMONSQUEEZY_STORE_ID

    // Validate required environment variables
    if (!apiKey) {
      logger.error('❌ CRITICAL: LEMONSQUEEZY_API_KEY environment variable is not configured!')
      return NextResponse.json(
        {
          error: 'Payment service not configured',
          details: 'LEMONSQUEEZY_API_KEY environment variable is missing',
    ...
    if (!storeId) {
      logger.error('❌ CRITICAL: LEMONSQUEEZY_STORE_ID environment variable is not configured!')
      return NextResponse.json(
        {
          error: 'Payment service not configured',
          details: 'LEMONSQUEEZY_STORE_ID environment variable is missing',
          timestamp: new Date().toISOString()
        },
        { status: 500 }
      )
      ...
      data: {
        ...
        relationships: {
          store: {
            data: {
              type: 'stores',
              id: storeId // Use environment variable
            }
          },
          variant: {
            data: {
