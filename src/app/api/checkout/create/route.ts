...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('📦 Request data:', { productKey, matchId })
    ...
    try {
      ...
      console.log('Session parsed:', { email: session.email, clientId: session.clientId })
    } catch (e) {
    ...
    if (!email || !clientId) {
      console.error('Missing email or clientId in session:', { email: !!email, clientId: !!clientId })
      return NextResponse.json(
    ...
    console.log('Product selected:', { productKey, credits: product.credits, variantId: product.variantId })
    ...
    if (!process.env.LEMONSQUEEZY_API_KEY) {
      ...
      console.error('❌ Available env vars:', Object.keys(process.env).filter(k => k.includes('LEMON')))
      return NextResponse.json(
    ...
    if (!product.variantId) {
      console.error('❌ Missing variantId for product:', productKey)
      return NextResponse.json(
    ...
    console.log('🚀 Creating checkout with payload:', JSON.stringify(requestBody, null, 2))
    ...
    if (!response.ok) {
      ...
      try {
        ...
        console.error('Raw error text:', error)
      }
