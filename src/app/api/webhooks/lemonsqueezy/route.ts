...
export async function POST(request: NextRequest) {
  try {
    ...
    // Get webhook secret from environment variable - NO FALLBACK for security
    const secret = process.env.LEMONSQUEEZY_WEBHOOK_SECRET

    if (!secret) {
      logger.error('🚨 CRITICAL: LEMONSQUEEZY_WEBHOOK_SECRET environment variable is not configured!')
      logger.error('🚨 Please set the webhook secret in your environment variables')
      return NextResponse.json(
        {
          error: 'Webhook secret not configured',
          details: 'LEMONSQUEEZY_WEBHOOK_SECRET environment variable is missing',
