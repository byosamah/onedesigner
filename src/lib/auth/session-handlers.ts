...
export async function validateSession(
  ...
  switch (type) {
    ...
    case 'ADMIN':
      // Verify admin email against whitelist from environment variable
      // Set ADMIN_EMAILS or ADMIN_EMAIL environment variable
      ...
      if (adminEmails.length === 0 || !adminEmails.includes(session.email.toLowerCase())) {
