...
Cypress.Commands.add('enterOTP', (otp: string) => {
  // Enter each digit into separate OTP input fields
  otp.split('').forEach((digit, index) => {
    cy.get(`input[name="otp-${index}"], input[id="otp-input-${index}"]`)
      .first()
      .type(digit)
  })
  cy.get('button[type="submit"]').contains(/verify|confirm/i).click()
})
